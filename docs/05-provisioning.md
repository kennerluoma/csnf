...
| `sanity.datasets`  | Sanity     | `PUT /projects/:id/datasets/production`, `…/development` `{aclMode:"public"}`                                                                     | PUT is idempotent                             |
...
Sanity and GitHub calls above were verified against current docs on 2026-09-16. Dataset creation is **PUT** with the name in the path. GitHub wants header `X-GitHub-Api-Version: 2026-03-10`. Cloudflare token needs **Workers Scripts: Edit** (and DNS: Edit on zones for doc 06); verify the Workers script endpoints before implementing.
