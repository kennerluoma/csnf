...
2. For each issue, in the brief's order: make the smallest change that resolves it in the way the site is already built (primitives, tokens, blocks, resolvers, routes). Reuse before adding. If the issue names a value (a breakpoint, a viewport unit, a size), use that value; do not re-derive it from the design. Where an issue and .agency/lessons.md disagree, the issue wins; say so in the PR body.
