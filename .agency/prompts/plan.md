...
5. "Content model": every content type this kind of site runs on gets a working, styled index and detail route and a studio entry, drawn or not: artworks, artists, exhibitions, events, news/articles, and the categories/tags/series that group them. The template ships them; say which the design styles directly and which inherit that styling. Never plan to leave a default route with starter styling.
